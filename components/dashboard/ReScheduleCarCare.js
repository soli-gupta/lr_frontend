...
export default function ReScheduleCarCare(props) {
    ...
    const rescheduleSellRequest = async (e) => {
        ...
        setTinyLoader(true)
        await axios.post(`${process.env.NEXT_PUBLIC_URL}user/reschedule-car-care-request`, formData, {
            headers: {
            ...
            .then(function (res) {
                if (res.data.status == 1) {
                    ...
                    if (router.pathname === '/dashboard/car-care') {
                        router.reload()
                    } else {
                        router.push('/dashboard/car-care')
                    }
                }
            })
                            ...
                            <div className='scroll-div only-time-slot'>
                                            ...
                                            weekCal && weekCal.map((date, i) => {
                                                ...
                                                return (
                                                    <div className='date' key={i}>
                                                        <input type="radio" name="slot_day" defaultValue={dateConverterForValue(`${date.day}`, `${date.month}`, `${date.year}`)} id={`selected_${i}`} d onChange={(e) => dateTimeHandle(e)} defaultChecked={props.data && props.data.slot_day ? dateConverterForValue(`${date.day}`, `${date.month}`, `${date.year}`) === props.data.slot_day ? true : false : ''} disabled={(date.day === day) && (hrs >= '16') ? true : false} />
                                ...
                                {/* <div className='style-box time-slot no-border add-time-date'>
                                    ...
                                    <div className='dates'>
                                        ...
                                        <div className='date'>
                                            <input type="radio" name="slot_time" id="time4" defaultValue="4 PM - 6 PM" onChange={(e) => dateTimeHandle(e)} defaultChecked={props.data && props.data.slot_time ? "4 PM - 6 PM" === props.data.slot_time ? true : false : ''} disabled={dateTime ? (dateTime.slot_day === today && hrs >= 14 ? true : false) : ''} />
