...
import PriceFilter from './filters/FiltersHead/PriceFilter';
import YearFilter from './filters/FiltersHead/YearFilter';
import KMSFilter from './filters/FiltersHead/KMSFilter';
...
export default function WebsiteHeader() {
    ...
    if (typeof window !== 'undefined' && pathname !== '/buy') {
    ...
    const HideSearch = () => {
        document.getElementById('header-filter').reset();
    ...
    const userProfileData = async () => {
        await axios.get(`${process.env.NEXT_PUBLIC_URL}user-profile`, {
            headers: {
    ...
    useEffect(() => {
        ...
        setToken(localStorage.getItem("lr-user-token"))
        userProfileData()
    ...
    }, [typeof window !== "undefined" ? localStorage.getItem("lr-user-token") : false, token]);


    const getCategoryValue = (id, name, index) => {
    ...
    const fetchCenters = useSWR(`${Url}fetch-all-service-centers`, fetcher);

    const user = useSWR(`${Url}user-profile`, fetch);

    userInfo = user !== undefined && user.data !== undefined && user.data.user !== undefined && user.data.user !== '' ? user.data.user : ''
    ...
    const applyAllFilters = (e) => {
        ...
        let $brandName = document.querySelectorAll('input[name="brand_name_header[]"]:checked');
        ...
        let $bodyType = document.querySelectorAll('input[name="body_type_header[]"]:checked');
        ...
        let $fuelType = document.querySelectorAll('input[name="fuel_type_header[]"]:checked');
        ...
        const $getPriceVal = document.getElementById('price-range-count-header').value;
        if ($getPriceVal > 0) {
        ...
        let year = '';
        const $getYearval = document.getElementById('year-range-count-header').value;
        if ($getYearval > 0) {
        ...
        let kms = '';
        const $getKMsVal = document.getElementById('kms-range-count-header').value;
        if ($getKMsVal > 0) {
    ...
    // console.log(fetchCenters.data);
    return (
        <>
            <header className={scroll ? "header-fix" : ""}>
                <div className="wrapper">
                    ...
                    <nav>
                        <ul>
                            <li className={`buy ${router.pathname === '/buy' ? 'active' : ''}`}><Link href="/buy">Buy</Link></li>
                            <li className={`sell ${router.pathname === '/sell' ? 'active' : ''}`}><Link href="/sell">Sell</Link></li>
                            <li className={`service ${router.pathname === '/service' ? 'active' : ''}`}><Link href="/service">Service</Link>

                                <div className='mega-menu'>
                                    <div className="repairshop commonm-gap bg-grey">
                                        <div className="wrapper">


                                            <div className="row">

                                                {serviceCategory && serviceCategory.map((category, i) => {
                                                    return (
                                                        <div className="width-20" key={i}>
                                                            <div className="spend-bx">
                                                                <a className='cursor-pointer desktop-style' onClick={() => getCategoryValue(category._id, category.name, i)}></a>
                            ...
                            <li className={`car-care ${router.pathname === '/car-care' ? 'active' : ''}`}><Link href="/car-care">Car Care</Link></li>
                            <li className={`service-package ${router.pathname === '/service-packages' ? 'active' : ''}`}><Link href="/service-packages">Service Packages</Link></li>
                            <li className={`extended-warranty ${router.pathname === '/extended-warranty' ? 'active' : ''}`}><Link href="/extended-warranty">Extended Warranty</Link></li>
                            <li className={`insurance ${router.pathname === '/insurance' ? 'active' : ''}`}><Link href="/insurance">Insurance</Link></li>
                            <li className={`loan ${router.pathname === '/loan' ? 'active' : ''}`}><Link href="/loan">Loans</Link></li>
                        </ul>
                    </nav>
                    ...
                    <div className="right">
                        <div className="phone">
                            <Link href="tel:+91 8410084100">+91 8410084100</Link>
                        </div>
                        <div onClick={OpenSearch} className="search">
                            <img src="/img/search.svg" alt="" />
                        </div>

                        <div className={`user-profile desktop-style ${userInfo.profile ? ' user-display-image' : userInfo.first_name ? 'user-display-image' : ''}`}>
                            {userProfile || token ? <> <Link className='desktop-style' href="/dashboard/personal-information">
                                {/* {userInfo.first_name && userInfo.profile ? 
                                    <img src={`${process.env.NEXT_PUBLIC_URL}${userInfo.profile}`} alt="" /> 
                                    : userInfo.first_name ? userInfo.first_name.charAt(0).toUpperCase(userInfo.first_name) : <img src="/img/profile-user.svg" alt="" />} */}
                                {userInfo.first_name ? userInfo.first_name.charAt(0).toUpperCase(userInfo.first_name) : <img src="/img/profile-user.svg" alt="" />}

                            </Link>
                            </> : <img className='desktop-style' src="/img/profile-user.svg" alt="" onClick={() => openLogin()} />}

                        </div>

                        <div className={`user-profile mobile-style ${userInfo.profile ? ' user-display-image' : userInfo.first_name ? 'user-display-image' : ''}`}>
                            {token ? <> <Link className='mobile-style' href="/dashboard/user-menu">
                                {/* {userInfo.first_name && userInfo.profile ? 
                                    <img src={`${process.env.NEXT_PUBLIC_URL}${userInfo.profile}`} alt="" /> 
                                    : userInfo.first_name ? userInfo.first_name.charAt(0).toUpperCase(userInfo.first_name) : <img src="/img/profile-user.svg" alt="" />} */}
                                {userInfo.first_name ? userInfo.first_name.charAt(0).toUpperCase(userInfo.first_name) : <img src="/img/profile-user.svg" alt="" />}
                            </Link>
                            </> : <img className='mobile-style' src="/img/profile-user.svg" alt="" onClick={() => openLogin()} />}

                        </div>


                        <div onClick={OpenHamburger} className="tgmenu">
            ...
            <div className={`${Hamburger ? "open" : ""} hamburger-menu`}>
                ...
                {Hamburger ? <div onClick={HideHam} className='out-side-close'></div> : ""}

                <div className='ham-close' onClick={HideHam}></div>

                <div className='ham-menu mobile-style'>
                    <h3>Main Menu</h3>
                    <ul>
                        <li onClick={HideHam} className={`buy ${router.pathname === '/buy' ? 'active' : ''}`}><Link href={`/buy`}>Buy</Link></li>
                        <li onClick={HideHam} className={`sell ${router.pathname === '/sell' ? 'active' : ''}`}><Link href={`/sell`}>Sell</Link></li>
                        <li onClick={HideHam} className={`service ${router.pathname === '/service' ? 'active' : ''}`}><Link href={`/service`}>Service</Link></li>
                        <li onClick={HideHam} className={`car-care ${router.pathname === '/car-care' ? 'active' : ''}`}><Link href={`/car-care`}>Car Care</Link></li>
                        <li onClick={HideHam} className={`service-packages ${router.pathname === '/service-packages' ? 'active' : ''}`}><Link href={`/service-packages`}>Service Pakages</Link></li>
                        <li onClick={HideHam} className={`extended-warranty ${router.pathname === '/extended-warranty' ? 'active' : ''}`}><Link href={`/extended-warranty`}>Extended Warranty</Link></li>
                        <li onClick={HideHam} className={`insurance ${router.pathname === '/insurance' ? 'active' : ''}`}><Link href={`/insurance`}>Insurance</Link></li>
                        <li onClick={HideHam} className={`loan ${router.pathname === '/loan' ? 'active' : ''}`}><Link href={`/loan`}>Loans</Link></li>
                    </ul>

                </div>

                <div className='ham-menu'>
                    <h3>Explore Luxury Ride</h3>
                    <ul>
                        <li onClick={HideHam} className={`${router.pathname === '/about-us' ? 'active' : ''}`}><Link href={`/about-us`}>ABOUT US</Link></li>
                        <li onClick={HideHam} className={`${router.pathname === '/luxury-ride-assurance' ? 'active' : ''}`}><Link href={`/luxury-ride-assurance`}>LUXURY RIDE ASSURANCE</Link></li>
                        <li onClick={HideHam} className={`${router.pathname === '/careers' ? 'active' : ''}`}><Link href={`/careers`}>CAREERS</Link></li>
                        <li onClick={HideHam} className={`${router.pathname === '/luxury-ride-buy-back' ? 'active' : ''}`}><Link href={`/luxury-ride-buy-back`}>LUXURY RIDE BUY BACK</Link></li>
                        <li onClick={HideHam} className={`${router.pathname === '/blog' ? 'active' : ''}`}><Link href={`/blog`}>BLOG</Link></li>
                        <li onClick={HideHam} className={`${router.pathname === '/contact-us' ? 'active' : ''}`}><Link href={`/contact-us`}>CONTACT US</Link></li>
                        <li onClick={HideHam} className={`${router.pathname === '/news-and-media' ? 'active' : ''}`}><Link href={`/news-and-media`}>NEWS & MEDIA</Link></li>
                    </ul>

                </div>

                <div className='locations  desktop-style'>

                    <h3>Our Locations</h3>

                    <div className='head-office'>
                        {
                            fetchCenters !== undefined && fetchCenters.data !== undefined && fetchCenters.data.experience_center !== undefined && fetchCenters.data.experience_center !== '' && fetchCenters.data.experience_center.map((center) => {
                                return (
                                    <div key={center._id}>
                                        {center.name === 'Head Office' ? <div>
                                            <h6>{center.name} <span>{center.city}</span></h6>
                                            {/* {console.log(countWords(center.address) <= 7 ? center.words : '')} */}
                                            <address>
                                                {center.address ? center.address : ''}
                                            </address>
                                            {center.name !== '' && center.name !== undefined && center.address !== '' && center.address !== undefined ?
                                                <Link target="_blank" rel="noopener noreferrer" href={`https://www.google.com/maps/search/Luxury Ride ${center.name} ${center.address}`} className="btn center arrow-style blueBdr"><span>Get Directions</span></Link> : ''}
                                        </div> : ''}
                                    </div>
                                )
                            })
                        ...
                        <Link href="#" className="btn center arrow-style blueBdr"><span>Get Directions</span></Link> */}
                    ...
                    <div className='all-adress'>
                        ...
                        {
                            fetchCenters !== undefined && fetchCenters !== '' && fetchCenters.data !== undefined && fetchCenters.data !== '' && fetchCenters.data.experience_center !== undefined && fetchCenters.data.experience_center !== '' && fetchCenters.data.experience_center.map((center) => {
                                if (center.name !== 'Head Office') {
                                    return (
                                        <div className='adress' key={center._id}>
                                            <h6>{center.name} <span>Experience Centre</span></h6>
                                            <address>
                                                {center.address}
                                            </address>

                                            {center.name !== '' && center.name !== undefined && center.address !== '' && center.address !== undefined ?
                                                <Link target="_blank" rel="noopener noreferrer" href={`https://www.google.com/maps/search/Luxury Ride ${center.name} ${center.address}`}><span>Get Directions</span></Link> : ''}
                                        </div>
                                    )
                                }
                            })
                ...
                <div className='socials'>
                    ...
                    <div className="social">
                        ...
                        <Link target='_blank' href="https://www.instagram.com/LuxuryRide.in/">
                            <i className="fa fa-instagram" aria-hidden="true"></i>
                        </Link>
                        <Link target='_blank' href="https://www.facebook.com/LuxuryRideGlobal">
                            <i className="fa fa-facebook" aria-hidden="true"></i>
                        </Link>
                        <Link target='_blank' href="https://www.youtube.com/channel/UC7_pAp2TGmq2z2DJ9T94D1g">
                            <i className="fa fa-youtube-play" aria-hidden="true"></i>
                        </Link>

                        <Link target='_blank' href="https://www.linkedin.com/company/luxuryrideindia/">
            ...
            <div className={`${CustomSearch ? "open" : ""} custom-search`}>

                {CustomSearch ? <div onClick={HideSearch} className='out-side-close'></div> : ""}


                <form onSubmit={applyAllFilters} method="POST" id="header-filter">

                    <div className='sticky-head'>

                        <div onClick={HideSearch} className='search-close'></div>

                        <h3>Search By Filters</h3>

                    </div>

                    <div className='search-box fule-type with-name'>
                        ...
                        {
                            fetchBrands !== undefined && fetchBrands.data !== undefined && fetchBrands.data.brands !== undefined && fetchBrands.data.brands.map((brand) => {
                                return (

                                    <div className="form-group" key={brand._id}>
                                        <input type="checkbox" id={`brand_${brand.slug}_header`} name="brand_name_header[]" defaultValue={brand.slug} className="brandBtn" />
                                        <label htmlFor={`brand_${brand.slug}_header`}>
                    ...
                    <div className='search-box'>
                        ...
                        <h4 className='gap'>Price Range (INR)</h4>

                        <PriceFilter min={1000000}
                            max={10000000}
                            step={100000}
                    ...
                    <div className='search-box fule-type'>

                        <h4>Body Type</h4>
                        <div className='search-bttomgap'>
                            {
                                fetchBodyTypes.data && fetchBodyTypes.data.body_type.map((bodyType) => {
                                    return (
                                        <div className="form-group body-type" key={bodyType._id}>
                                            <input type="checkbox" id={`body_${bodyType.body_slug}_header`} onChange={handleCheckBox} defaultValue={bodyType.body_slug} name="body_type_header[]" />
                                            <label htmlFor={`body_${bodyType.body_slug}_header`}>
                                                <p>
                                                    <span>{bodyType.body_name}</span>
                                                    <img src={`${process.env.NEXT_PUBLIC_URL}${bodyType.body_image}`} />
                                                </p>
                                            </label>
                                        </div>
                                    )
                                ...
                                fetchFuelTypes.data && fetchFuelTypes.data.fuel_type.map((fuel) => {
                                    return (
                                        <div className="form-group body-type" key={fuel._id}>
                                            <input type="checkbox" id={`fuel_${fuel.fuel_slug}_header`} onChange={handleCheckBox} defaultValue={fuel.fuel_slug} name="fuel_type_header[]" />
                                            <label htmlFor={`fuel_${fuel.fuel_slug}_header`}>
                                                <p>
                    ...
                    <div className='search-box'>
                        ...
                        <h4 className='gap'>KMs Driven</h4>
                    ...
                    <div className='apply-filter'>
                        <button className="btn reverse-border-btn">
            ...
            <LoginForm Login={Login} closePop={closePop} redirectRoute={router.pathname} />


            <div className={`${scrollDir == "down" ? "hide" : ""} mobile-menu`}>

                <ul>

                    <Swiper {...swiperSettingMenu}>

                        <SwiperSlide> <li className={` ${router.pathname === '/buy' ? 'active' : ''}`} ><Link href="/buy"><span> <img src="/img/mob-buy.svg" alt="Luxury Ride" /></span>Buy</Link></li>
                        </SwiperSlide>
                        <SwiperSlide>  <li className={` ${router.pathname === '/sell' || router.pathname === '/sell/sellMobileFilter' ? 'active' : ''}`} ><Link href="/sell"><span> <img src="/img/mob-sell.svg" alt="Luxury Ride" /></span> Sell</Link></li></SwiperSlide>
                        <SwiperSlide> <li className={` ${router.pathname === '/service' || router.pathname === '/service/serviceMobileFilter' ? 'active' : ''}`}><Link href="/service"><span> <img src="/img/mob-service.svg" alt="Luxury Ride" /></span>  Service</Link></li> </SwiperSlide>
                        <SwiperSlide>   <li className={` ${router.pathname === '/car-care' || router.pathname === '/car-care/carCareMobileFilter' ? 'active' : ''}`} ><Link href="/car-care"><span> <img src="/img/mob-car-care.svg" alt="Luxury Ride" /></span>  Car Care</Link></li></SwiperSlide>
                        <SwiperSlide> <li className={`${router.pathname === '/service-packages' ? 'active' : ''}`} ><Link href="/service-packages"><span> <img src="/img/mob-serive-package.svg" alt="Luxury Ride" /></span>  Service Pakages</Link></li></SwiperSlide>
                        <SwiperSlide> <li className={`${router.pathname === '/extended-warranty' ? 'active' : ''}`} ><Link href="/extended-warranty"><span> <img src="/img/mob-extend.svg" alt="Luxury Ride" /></span> Extended Warranty</Link></li> </SwiperSlide>
                        <SwiperSlide>  <li className={`${router.pathname === '/insurance' ? 'active' : ''}`} ><Link href="/insurance"><span> <img src="/img/mob-insurance.svg" alt="Luxury Ride" /></span> Insurance</Link></li></SwiperSlide>
                        <SwiperSlide> <li className={`${router.pathname === '/loan' ? 'active' : ''}`}><Link href="/loan"><span> <img src="/img/mob-loans.svg" alt="Luxury Ride" /></span>  Loans</Link></li></SwiperSlide>
