...
function Index() {
	...
	const { data: getOrdersArray, mutate, size, setSize, isValidating, isLoading, error } = useSWRInfinite((index) => {
		...
		return `${Url}user/get-order-details-by-user?type=${type}&orderType=service-package&page=${aofs}`;
	}
		, fetcher);
	...
	const allServicePackageHandler = () => {
		router.push('/dashboard/service-package');
	}

	const pendingServicePackageHandler = () => {
		router.push('?type=pending');
	}

	const activeServicePackageHandler = () => {
		router.push(`?type=active`);
	}

	const expiredServicePackageHandler = () => {
	...
	return (
		<div>
			<WebHead pageTitle={`Service Package`} />
			...
			<div className='user-dashboard'>
				...
				<div className='user-mob-back mobile-style'>
					...
					<Link className='back-link' href="user-menu">Service Package</Link>
					...
					<div className='user-bgBox'>
						...
						<div className='user-right test-drive-user sell-req'>
							...
							<div className='formbx-white p-0 user-style'>
								...
								<div className="border-heading"><h3>Service Package</h3></div>
										...
										{getOrders !== undefined && getOrders.length > 0 ?
												...
												<div className='car-user-filter'>
													...
													<ul>
														<Swiper {...swiperSettingMenuUser}>
															<SwiperSlide>	<li className={`${type === '' || type === undefined ? 'active' : ''}`} onClick={allServicePackageHandler} >All</li></SwiperSlide>

															<SwiperSlide>		<li className={`${type === 'pending' ? 'active' : ''}`} onClick={pendingServicePackageHandler} >Pending</li></SwiperSlide>

															<SwiperSlide><li className={`${type === 'active' ? 'active' : ''}`} onClick={activeServicePackageHandler}>Active Package</li></SwiperSlide>

															<SwiperSlide> <li className={`${type === 'expired' ? 'active' : ''}`} onClick={expiredServicePackageHandler}>Expired Package</li></SwiperSlide>
														</Swiper>
																			...
																			<div className='grey-bg'>
																				<div className="prductSec01">
																					...
																					<div className="row">
																						...
																						{/* <div className="price-detail col-md-3 no-border text-right">
																								...
																								<div className="price">INR <span> 97,290/-</span></div>
																					...
																					<div className='price-detail col-md-3 only-btn'>
																						{order.form_step !== 5 || order.order_status === 4 ? <>
																							<Link className='btn  reverse' href={`/dashboard/service-package/${order._id}`}>Complete Your Form</Link>
																						</> : <></>}
																					</div>
																				...
																				{/* <div className='car-user-info two-colum align-top gap-b'>
																						...
																						<div className='car-div-dashboard no-border'>
																							...
																							<h4 className="blue-color">Diamond Package</h4>

																							<p className='no-icon'>This package consist of <span>2 Standard Services & 1 Comprehensive Service</span> and <span>valid upto 3 years or 45,000 KM</span> whichever comes first.</p>
																						...
																						{order.order_status === 3 ? <div className="car-div-dashboard no-block actions"><div><a href="" className="btn reverse green-btn-user">Book Another Package</a></div></div> : ''}

																					</div>

																					<div className={`pakage-info ${order.order_status === 3 ? 'grey-text' : ''}`}>
																				...
																				{/* {console.log(order.service_center_address !== '' && order.service_center_address !== undefined ? "Done" : "Error")} */}
																				{
																					order.service_center_address !== '' && order.service_center_address !== undefined ?
																						...
																						<div className='car-user-info align-top'>
																							...
																							<div className='car-div-dashboard'>
																								...
																								<h3 className={`${order.order_status === 3 ? 'greay-head' : ''}`}>Preferred Service Centre</h3>
																								...
																								{order.order_status === 1 || order.order_status === 4
																									? <>
																										...
																										<div className='user-email'>
																											...
																											{order.service_center_address !== '' && order.service_center_address !== undefined && order.service_center_name !== '' && order.service_center_name !== undefined ?
																											...
																											{order.order_status === 1 || order.order_status === 4 ?
																												...
																												: <>
																													...
																													<div className="grey-text calender">
																														...
																														{order.vehicle_inspaction_date !== undefined && order.vehicle_inspaction_date !== '' && order.vehicle_inspaction_time !== undefined && order.vehicle_inspaction_time !== '' ?
											...
											<>
												...
												<div className='no-data-added' style={{ display: "block" }}>
													...
													<h6>You haven’t booked any service package yet</h6>

													<Link className='btn arrow-style blue-btn' href={`/service-packages`}>
