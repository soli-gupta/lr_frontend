...
function FinalExtendedWarrantyDetails() {
    ...
    const submitPayAtWorkaShop = async (e) => {
        ...
        await axios.patch(`${Url}update-service-package-order/${orderId}`, getServicePackageCashPayment, {
            ...
            if (res && res.data.status === 1) {
                router.push(`/dashboard/extended-warranty/thank-you/${res.data.orderId}`);
            }
        }).catch((e) => {
            popUp(true);
            if (e && e.message) {
    ...
    return (
        <>
            <WebHead pageTitle={`${order.order_brand_name} ${order.order_model_name} ${order.order_variant_name} | Booked Extended Warranty`} />
            {/* <WebsiteLayout> */}
                        ...
                        <div className="row">
                            <div className="col-md-7">
                                ...
                                <div className='formbx-white add-address' style={{ display: order.form_step >= 2 ? "block" : selectAddressTab === false ? "none" : "block" }}>
                                    <h3 style={{ display: order.form_step === 2 ? "block" : addressRadioAndAddTab === false ? "none" : "block" }}>Select/Add address For Vehicle Pickup</h3>
                                ...
                                <div className='formbx-white add-time-date' style={{ display: order.form_step >= 3 ? "block" : vehicleInspactionDateTab == false ? "none" : "block" }}>
                                    <h3 style={{ display: order.form_step === 3 ? "block" : vehicleInspactionDateTab == false ? "none" : "block" }}>Select Vehicle Pickup Date</h3>
                            ...
                            <div className="col-md-5">
                                <div className="sticky-div">
                                    ...
                                    {/* <div className='formbx-white payment' style={{ display: order.form_step >= 4 ? "block" : paymentsTab == false ? "none" : "block" }}>
                                        <h3>Payments</h3>
                                        <div className='nextstep' style={{ display: order.form_step >= 4 ? "block" : paymentsTab == false ? "none" : "block" }}>
