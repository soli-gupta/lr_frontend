...
import AllServiceCenter from '@/components/AllServiceCenter';
import PixelLoader from '@/components/PixelLoader';

const fetcher = (url) => axios.get(url).then(res => res.data)
function CarCareIndex() {
	...
	const serviceCategory = useSWR(`${process.env.NEXT_PUBLIC_URL}user/car-care-categories`, fetcher);
	...
	const hidePopup = () => {
		...
		document.body.classList.remove('hide-scroll-poup-new');
		document.getElementById('carCareFormData').reset();
	}
		...
		await axios.get(`${process.env.NEXT_PUBLIC_URL}model-by-brand-id?id=${brandId}`)
			...
			.catch(function (error) {
				...
				if (error.response && error.response.data && error.response.data.status === 0) {
					...
					setCallSuccessMessage(error.response.data.message);
					// alert(error.response.data.message);
				}
			});
	...
	const checkFormChange = () => {
		let forms = document.getElementById('carCareFormData')
		let error = 0;
	...
	const getCategoryValue = (slug, id, name, index) => {
		...
		router.push({ pathname: `/car-care/car-care-details`, query: { id: id, name: name, index: index } }, '/car-care/car-care-details')
	}
	const saveDataAfterLogin = async () => {
		await axios.post(`${process.env.NEXT_PUBLIC_URL}user/save-user-car-care`, formDatas, {
			...
			.then(function (res) {
				if (res.data.status == 1 && res.data.insertId) {
					setTinyLoader(false);
	...
	const submitData = async (e) => {
		let formData = new FormData(document.getElementById('carCareFormData'))
		if (colorValue) {
			formData.append('color', colorValue)
		}
		setFormData(formData)
		if (brandName === '' || modelName === '' || colorName === '') {
	...
	const cmspage = useSWR(`${process.env.NEXT_PUBLIC_URL}cms-page/car-care`, fetcher);
	...
	return (
		<>
			<WebHead pageTitle="Car-Care" />
			{/* <PixelLoader /> */}
			{/* <WebsiteLayout> */}


			<div className="home-banner inner-banner mobile-view-hide">
				<img src="img/car-care-banner.png" alt="" />

				<div className="filter-section care-filtercare">

					<h2>Detailing Services That Fulfill All Your Needs</h2>
					<div className='car-search' ref={ref}>
						<form method='POST' id="carCareFormData" onInput={checkFormChange}>
							...
							<div className={`div-data right-style div-data ${colorName || colorValue ? 'added' : ''}`} onClick={openDivColor}>
								<div className='data'>
									<span>Color</span>
									<i>{colorName ? colorName : colorValue}</i>
								</div>

								<div className="car-data" style={{ display: `${divColor === true ? 'block' : 'none'}` }} onMouseLeave={hideColorPop}>

									<h5>Select Color</h5>

									<div className="color-filter">
										<div className="d-flex flex-wrap">
											{data !== undefined && data.data.map((color, i) => {
												return (
			...
			<div className="inner-mobile-banner desktop-hide-div mobile-show-div bottom-text">
				<img src="/img/car-care-banner-mobile.png" alt="" />
				<div className="mobile-txt-banner">
					<div className="wrapper">
						<h2>Detailing Services That Fulfill All Your Needs
						</h2>
						<Link className="btn" href="car-care/carCareMobileFilter">Get Started</Link>
					</div>
				</div>
			</div>
			...
			<section className="repairshop commonm-gap bg-grey">
				<div className="wrapper">
					<div className="center-text">
						<h2> Transform Your Car The Way You Want</h2>
						<p>Maintain your car hygiene and cleanliness with Luxury Ride. We offer a variety of care<br /> care solutions to ensure a driving experience like never before!</p>
					</div>
					...
					<div className="row justify-content-center">
						...
						<div className="width-20">
							<div className="spend-bx">
								<a href='#'></a>
								<div className="img-spend">
									<img src="img/ceramic-coating-icon.svg" alt="" />
								</div>
			...
			<FrequentlyAskedQuestions heading="Frequently Asked Questions On Car Care" pageType={cmspage.data !== undefined && cmspage.data.cms_page.slug} />
			<div className={`${Popup ? "open-popup" : ""} common-popup`}>
				...
				<div className='popup-inner'>
					...
					<div className='style-box'>
						...
						<div className='locations'>
							...
							<div className='location'>
								...
								<input id='adress1' type="radio" name="radio-group" />
							...
							<div className='location'>
								...
								<input id='adress2' type="radio" name="radio-group" />
								<label htmlFor="adress2">
									<span>Karnal</span>
					...
					<div className='style-box no-border add-time-date'>
						...
						<div className='dates time-pd '>
							...
							<div className='date'>
								<input type="radio" name="date-slot" id="date" />
								<label htmlFor="date" className="option">
									24 Jan <span>Today</span>
								</label>
							</div>
							<div className='date'>
								<input type="radio" name="date-slot" id="date1" />
								<label htmlFor="date1" className="option">
									25 Jan <span>Tomorrow</span>
								</label>
							</div>
							<div className='date'>
								<input type="radio" name="date-slot" id="date2" />
								<label htmlFor="date2" className="option">
									26 Jan <span>Thursday</span>
								</label>
							</div>
							<div className='date'>
								<input type="radio" name="date-slot" id="date3" />
								<label htmlFor="date3" className="option">
									27 Jan <span>Friday</span>
								</label>
							</div>
							<div className='date'>
								<input type="radio" name="date-slot" id="date4" />
								<label htmlFor="date4" className="option">
									28 Jan <span>Saturday</span>
								</label>
							</div>
							<div className='date'>
								<input type="radio" name="date-slot" id="date5" />
								<label htmlFor="date5" className="option">
									29 Jan <span>Sunday</span>
								</label>
							</div>
							<div className='date'>
								<input type="radio" name="date-slot" id="date6" />
								<label htmlFor="date6" className="option">
									30 Jan <span>Monday</span>
								</label>
							</div>
							<div className='see-all'>See All Dates</div>
						</div>
					</div>
			...
			<LoginForm Login={Login} logInHeading="Log in" closePop={closePop} redirectRoute='/car-care/final-details-address' submitDataAfterSubmitLogIn={saveDataAfterLogin} loginId={`book-car-care`} />
