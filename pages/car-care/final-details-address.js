...
import { weekDatesForForms, dateConverterForValue, capitalizeFirstLetter, ButtonSpinner, onlyAlphaValidation } from '@/components/Helper';
...
export default function CareOwnerInfo() {
    ...
    const [addDateTimeSlot, setAddDateTimeSlot] = useState(true)
    ...
    const [choosePickupAddress, setChoosePickUpAddress] = useState('')
    ...
    const saveExpericenceCenter = async () => {
        if (serviceCenterName === '') {
            document.getElementById('serviceCenterError').innerHTML = 'Please select preferred car care centre.';
        ...
        setTinyLoader(true);
        await axios.post(`${process.env.NEXT_PUBLIC_URL}user/update-user-car-care/${id}`, data, {
            headers: {
    ...
    const carDetail = useSWR(`${Url}user/edit-user-car-care?id=${id}`, fetch)

    const getStates = useSWR(`${Url}states`, fetcher);
    const serviceCenters = useSWR(`${Url}fetch-car-care-service-center`, fetcher);
    ...
    const saveSelectedAddress = async (e) => {
        ...
        setTinyLoader(true);
        await axios.post(`${process.env.NEXT_PUBLIC_URL}user/update-user-car-care/${id}`, getUserAddressData, {
            headers: {
                'token': typeof window !== "undefined" ? localStorage.getItem("lr-user-token") : false,
            }
        })
    ...
    const saveDateTimeSlot = async (e) => {
        ...
        setTinyLoader(true);
        await axios.post(`${process.env.NEXT_PUBLIC_URL}user/update-user-car-care/${id}`, formData, {
            headers: {
                'token': typeof window !== "undefined" ? localStorage.getItem("lr-user-token") : false,
            }
        })
    ...
    const savePickUpPersonDetail = async (e) => {
        ...
        if (e.target.pickup_person_mobile.value.length <= 0) {
            document.getElementById('errorCarPickupPersonMobile').innerHTML = 'Please enter your mobile number';
            document.getElementById("errorCarPickupPersonMobile").style.display = "block";
            setTimeout(() => {
                document.getElementById('errorCarPickupPersonMobile').innerHTML = '';
                document.getElementById("errorCarPickupPersonMobile").style.display = "none";
            }, 3000);
            createError++;
        }

        if (e.target.pickup_person_mobile.value.length != 10) {
            document.getElementById('errorCarPickupPersonMobile').innerHTML = 'Please enter 10 digits mobile number';
            document.getElementById("errorCarPickupPersonMobile").style.display = "block";
            setTimeout(() => {
                document.getElementById('errorCarPickupPersonMobile').innerHTML = '';
                document.getElementById("errorCarPickupPersonMobile").style.display = "none";
        ...
        const formData = { ...{ 'pickup_person_name': pickupPersonFirstName + '' + pickupPersonLastName, 'pickup_person_mobile': pickupPersonMobile ? pickupPersonMobile : userData && userData.mobile }, ...form_step }
        setTinyLoader(true);
        await axios.post(`${process.env.NEXT_PUBLIC_URL}user/update-user-car-care/${id}`, formData, {
            headers: {
                'Content-Type': 'application/json',
    ...
    const submitCarCarePayment = async () => {
        setTinyLoader(true);
        await axios.post(`${process.env.NEXT_PUBLIC_URL}user/update-user-car-care/${id}`, { 'step_form': 6 }, {
            headers: {
                'Content-Type': 'application/json',
                'token': typeof window !== "undefined" ? localStorage.getItem("lr-user-token") : false,
            }
            ...
            .then(function (res) {
                if (res.data.status == 1) {
                    setTinyLoader(false);
                    router.push(`/car-care/thank-you?id=${id}`)
                }
            })
            .catch(function (e) {
                setTinyLoader(false)
    ...
    const selectServiceAddToCart = async () => {
        await axios.get(`${process.env.NEXT_PUBLIC_URL}user/get-car-care-by-order-id/${id}`, {
            headers: {
                'token': typeof window !== "undefined" ? localStorage.getItem("lr-user-token") : false,
            }
        })
    ...
    return (
        <>
                    ...
                    <div className="wrapper">
                        <div className="row">
                            <div className="col-md-7">
                                    ...
                                    <div className='vehicle-list-detail'>
                                        {carDetail.data !== undefined && carDetail.data && carDetail.data.data && carDetail.data.data.data &&
                                            <ul>
                                                ...
                                                <li>Color<span>{carDetail.data.data.data.color}</span></li>
                                            </ul>
                                        }
                                ...
                                <div className='formbx-white preferred-service-center'>
                                    <div style={{ display: `${step1 === true ? 'block' : 'none'}` }}>
                                        <form method='POST' id="centerForm">
                                            ...
                                            <h3>Select Preferred Car Care Centre</h3>
                                                        ...
                                                        return (
                                                            <li key={center._id}>
                                                                ...
                                                                <label htmlFor={`select-service_${center.name}`} className="option">
                                                                    ...
                                                                    <div className='addresstype'>{center.name === 'Head Office' ? 'Delhi (Saket)' : center.name === 'Delhi' ? 'Delhi (Alipur)' : center.name}</div>
                                                                </label>
                                                            </li>
                                                        )
                                                    })
                                        ...
                                        <ul>
                                            <li>
                                                ...
                                                <div className='editinfo-owner'>
                                                    <h4>Preferred Car Care Centre</h4>
                                ...
                                <div className='formbx-white add-time-date' style={{ display: `${step5 === true ? 'block' : 'none'}` }}>
                                    <form id="dayAndTimeForm" method='POST' onSubmit={(e) => saveDateTimeSlot(e)} >
                                        <div className='nextstep' style={{ display: `${step6 === true ? 'block' : 'none'}` }}>
                                            <h3>Select Date For Car Care</h3>
                                                        ...
                                                        weekCal && weekCal.map((date, i) => {
                                                            return (
                                                                <div className='date' key={i}>
                                                                    ...
                                                                    <label htmlFor={`selected_${i}`} className={`option ${(date.day === day) && (hrs >= '15') ? 'disable-time' : ''}`}> {date.day} {date.month}  <span>{date.weekDay}</span>
                                                                    </label>
                                                                </div>
                                                            )
                                                        })
                                ...
                                <div className='formbx-white ask-car-pickup' style={{ display: `${step8 === true ? 'block' : 'none'}` }}>
                                            ...
                                            <div className='row'>
                                                <div className='col-md-6'>
                                                    <div className='form-input'>
                                                        ...
                                                        <input type='text' id="pickup_person_first_name" name="pickup_person_first_name" placeholder='Enter your first name' onChange={(e) => setPickupPersonFirstName(e.target.value)} defaultValue={userData ? userData.first_name : pickupPersonFirstName} />
                                                ...
                                                <div className='col-md-6'>
                                                    <div className='form-input'>
                                                        ...
                                                        <input type='text' id="pickup_person_last_name" name="pickup_person_last_name" placeholder='Enter your last name' onChange={(e) => setPickupPersonLastName(e.target.value)} defaultValue={userData ? userData.last_name : pickupPersonLastName} />
                                                ...
                                                <div className='col-md-6'>
                                                    <div className='form-input'>
                                                        ...
                                                        <input type='text' name="pickup_person_mobile" className='numberonly' placeholder='Enter your mobile number*' onChange={(e) => setPickupMobile(e.target.value)} maxLength={10} defaultValue={userData ? userData.mobile : pickupPersonMobile} />
                                                        <small id="errorCarPickupPersonMobile" className="error"></small>
                                                    </div >
                                                </div >
                                                <div className='col-md-12'>
                                                    <div className='form-input mg-0'>
                                    ...
                                    <div className="nextstep">
                                        <div className='formbx-white p-0 mobile-view-hide'>
                                            ...
                                            <div className='vehicle-list-detail'>
                                                {carDetail.data !== undefined && carDetail.data && carDetail.data.data && carDetail.data.data.data ?
                                                    <ul>
                                                        {console.log(carDetail.data.data.data.brand_name)}
                                                        <li>Brand<span>{carDetail.data.data.data.brand_name}</span></li>
                                                        <li>Model<span>{carDetail.data.data.data.model_name}</span></li>
                                                        <li>Color<span>{carDetail.data.data.data.color}</span></li>
                                                    </ul>
                                                        ...
                                                        return (
                                                            <>
                                                                <li key={i}>
                                                                    ...
                                                                    <div className='right-txt blue-txt-order'>INR   {addService && addService.car_care_price ? addService.car_care_price : ''}/-
                                                                        <br />
            ...
            {LearMore ? <LearnMore closePop={closePop} />
                : ""
            }
